...
def upload_log(log:dict):
    ...
    for i in log["log"]:
        query1 = "match x=(p:project{PID:$pid})-[r:PHaveT]->(t:target{TID:$tid}) set r.Time_to_Observe={ramain:$remain} return r.Time_to_Observe"
        query2 = "match x=(p:project{PID:$pid})-[r:PHaveT]->(t:target{TID:$tid}) return r.Time_to_Observe"
        ...
        graph.run(query1, pid = int(i['pid']),tid = int(i['tid']), remain = remain_time - observe_time)
